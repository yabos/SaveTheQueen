...
public class A3_Tiles_Importer_Window : EditorWindow
{
    ...
    [MenuItem("Tools/RPGM Importer/Import A3 Tiles")]
    public static void ShowWindow()
    {
        EditorWindow.GetWindow<A3_Tiles_Importer_Window>(false, "A3 Tiles");
    }
        ...
        int mini_tile_w, int mini_tile_h, int wBlock, int hBlock, bool generate_sprite_sheet_image)
    {
        ...
        for (int i = 0; i < sub_blocks_wall_to_import.Count; i++)
        {
            ...
            string tile_folder_path = string.Format(@"{0}\Wall_{1}_{2}", loaded_file_image_path, Path.GetFileNameWithoutExtension(path), i);
            ...
            if (generate_sprite_sheet_image)
            {
                ...
                string sprite_sheet_path = string.Format(@"{0}\Wall_{1}_{2}.png", loaded_file_image_path, Path.GetFileNameWithoutExtension(path), i);
