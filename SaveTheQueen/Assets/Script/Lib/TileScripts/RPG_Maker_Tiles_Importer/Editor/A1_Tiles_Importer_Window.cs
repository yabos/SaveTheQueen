...
public class A1_Tiles_Importer_Window : EditorWindow
{
    ...
    /// List of the sub pieces from the tile set
    /// </summary>
    List<Texture2D> sub_blocks_floating;

    /// <summary>
    /// List of the sub pieces from the tile set
    /// </summary>
    List<Texture2D> sub_blocks_twister;

    /// <summary>
    /// List of boolean to select the block to import
    /// </summary>
    List<bool> sub_blocks_water_to_import;

    /// <summary>
    /// List of boolean to select the block to import
    /// </summary>
    List<bool> sub_blocks_floating_to_import;

    /// <summary>
    /// List of boolean to select the block to import
    /// </summary>
    List<bool> sub_blocks_twister_to_import;
    ...
    [MenuItem("Tools/RPGM Importer/Import A1 Tiles")]
    public static void ShowWindow()
    {
        EditorWindow.GetWindow<A1_Tiles_Importer_Window>(false, "A1 Tiles");
    }
        ...
        int mini_tile_w, int mini_tile_h, int wBlock, int hBlock, bool generate_sprite_sheet_image)
    {
        ...
        for (int sub_block_count = 0; sub_block_count < sub_blocks_twister_to_import.Count; sub_block_count++)
        {
            ...
            string tile_folder_path = string.Format(@"{0}\Water_Fall_{1}_{2}", loaded_file_image_path, Path.GetFileNameWithoutExtension(path), sub_block_count);
            ...
            if (generate_sprite_sheet_image)
            {
                ...
                string sprite_sheet_path = string.Format(@"{0}\Water_fall_{1}_{2}.png", loaded_file_image_path, Path.GetFileNameWithoutExtension(path), sub_block_count);
            ...
            else
            {
                ...
                foreach (Texture2D tile in Tiles_A1_Utility.Generate_Final_Tiles_A1_Twister(mini_tile_w, mini_tile_h * 2, left_mini_tiles, right_mini_tiles, rule_tiles))
                {
                    ...
                    string tile_file_path = string.Format(@"{0}\Water_Fall_{1}_{2}_{3:000}.png", tile_folder_path,
        ...
        int mini_tile_w, int mini_tile_h, int wBlock, int hBlock, bool generate_sprite_sheet_image)
    {
        ...
        for (int sub_block_count = 0; sub_block_count < sub_blocks_water_to_import.Count; sub_block_count++)
        {
            ...
            for (int i = sub_block_count * 3; i < sub_block_count * 3 + 3; i++)
            {
                ...
                //create the directory for the final images
                string tile_folder_path = string.Format(@"{0}\Water_{1}_{2}", loaded_file_image_path, Path.GetFileNameWithoutExtension(path), i);
                ...
                if (generate_sprite_sheet_image)
                {
                    ...
                    string sprite_sheet_path = string.Format(@"{0}\Water_{1}_{2}.png", loaded_file_image_path, Path.GetFileNameWithoutExtension(path), i);
                ...
                else
                {
                        ...
                        bottom_left_mini_tiles, bottom_right_mini_tiles, top_left_mini_tiles, top_right_mini_tiles, rule_tiles))
                    {
                        ...
                        string tile_file_path = string.Format(@"{0}\Water_{1}_{2}_{3:000}.png", tile_folder_path,
                            Path.GetFileNameWithoutExtension(path), sub_block_count, tiles_counter);
