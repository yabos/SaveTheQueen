...
namespace LitJson
{
    ...
    public class JsonReader
    {
        ...
        private JsonReader (StringReader reader, bool owned)
        {
            if (reader == null)
        ...
        private void ProcessNumber (string number)
        {
            if (number.IndexOf ('.') != -1 ||
                ...
                float n_single;
                if (Single.TryParse(number, out n_single))
                {
                    token = JsonToken.Single;
                    token_value = n_single;
        ...
        public bool Read ()
        {
            ...
            while (true) {
                ...
                if (current_symbol == current_input) {
                    if (! ReadToken ()) {
                        if (automaton_stack.Peek () != (int) ParserToken.End)
                            UnityEngine.Debug.Log(//throw new JsonException (
                ...
                try {
                    ...
                    UnityEngine.Debug.Log(string.Format("Invalid token '{0}' in input string", (char)current_input));
                    return false;
                }
