...
public class Tiles_A1_Utility {
        ...
        for (int y = 0; y < 4; y++)
        {
            for (int x = 0; x < 8; x++)
            {
                ...
                if (x == 7 || (x == 3 && (y == 2 || y == 3)))
                    ...
                    floating_blocks.Add(sub); //add the 2 floating element
                else
        ...
        Dictionary<byte, int> rule_tiles, int wBlock)
    {
        ...
        string atile_path = string.Format(@"{0}\{1}\{2}.asset", Tiles_Utility.Auto_Tile_Folder_Path, Path.GetFileNameWithoutExtension(source_File_Path), Path.GetFileNameWithoutExtension(frame1));
    ...
    {
        //create the auto tile for the animation water 
        string atile_path = string.Format(@"{0}\{1}\{2}.asset", Tiles_Utility.Auto_Tile_Folder_Path, Path.GetFileNameWithoutExtension(source_File_Path),
