...
namespace Lib.UniBt.Editor
{
    public static class BehaviorTreesEditorUtility
    {
        ...
        {
            foreach (Node child in bt.nodes)
            {
                if (child == null || child.Name == "Root")
                    continue;

                if (child.parentNode == null || child == child.parentNode)
                {
                    child.position = new Rect(position.x, position.y, BehaviorTreesEditorStyles.NodeNormalWidth, BehaviorTreesEditorStyles.NodeNormalHeight);
                    /*
                    if (child.Name.Equals("Task"))
                    {
                        child.position = new Rect(position.x, position.y, BehaviorTreesEditorStyles.NodeNormalWidth, BehaviorTreesEditorStyles.NodeNormalHeight);
                    }
                    if (child.Name.Equals("Selector"))
                    {
                        child.position = new Rect(position.x, position.y, BehaviorTreesEditorStyles.NodeNormalWidth, BehaviorTreesEditorStyles.NodeNormalHeight);
                    }
                    if (child.Name.Equals("Sequence"))
                    {
                        child.position = new Rect(position.x, position.y, BehaviorTreesEditorStyles.NodeNormalWidth, BehaviorTreesEditorStyles.NodeNormalHeight);
                    }
                    if (child.Name.Equals("Decorator"))
                    {
                        child.position = new Rect(position.x, position.y, BehaviorTreesEditorStyles.NodeNormalWidth, BehaviorTreesEditorStyles.NodeNormalHeight);
                    }
                    if (child.Name.Equals("Service"))
                    {
                        child.position = new Rect(position.x, position.y, BehaviorTreesEditorStyles.NodeNormalWidth, BehaviorTreesEditorStyles.NodeNormalHeight);
                    }
                     */
        ...
        public static void UnlinkNode(Node node, BehaviorTrees bt)
        {
            if (node.parentNode != null)
                node.parentNode.childNodes = ArrayUtility.Remove<Node>(node.parentNode.childNodes, node);
        ...
        {
            EventType eventType = UnityEngine.Event.current.type;
        ...
        {
            if (UnityEngine.Event.current.type != EventType.Used)
                UnityEngine.Event.current.type = type;
        }
